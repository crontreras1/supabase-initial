        ...
        <Link to={ `profile/${data.data.id_profile}` }>
            <div className='cursor-pointer w-72 h-96 p-3 ring-1 ring-inset ring-gray-500/10'>
                ...
                <div className='h-1/2 flex flex-col justify-evenly'>
                    { data.data.first_name ? <p className="text-2xl px-2.5 font-semibold leading-7 text-gray-900">{ `${data.data.first_name} ${data.data.last_name}` }</p> : <span className='hidden' ></span> }
                    ...
                    <div className="flex justify-evenly items-center text-sm">
                        ...
                        { data.data.free_class ? <p className="p-0 rounded-full flex justify-center w-24 text-xs font-bold leading-6 text-focus bg-focus/40">1ra Clase Gratis</p> : <span className='hidden'></span> }
                    </div>
                </div>
            </div>
        </Link>
